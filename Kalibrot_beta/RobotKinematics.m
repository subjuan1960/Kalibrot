...
classdef RobotKinematics
    ...
    methods
        ...
        function obj = RobotKinematics(n_joints, types, T_init,T_tool)
            ...
            obj.m_DH_params = sym('m_DH_params',[5*n_joints,1]);
...
% With Beta parameter
T = sym(T);
T(1,1) = cos(beta)*cos(theta);
T(1,2) = sin(alpha)*sin(beta) - cos(alpha)*cos(beta)*sin(theta);
T(1,3) = cos(alpha)*sin(beta) + cos(beta)*sin(alpha)*sin(theta);
T(1,4) = d*sin(beta) + a*cos(beta)*cos(theta);
...
T(3,1) = -sin(beta)*cos(theta);
T(3,2) = cos(beta)*sin(alpha) + cos(alpha)*sin(beta)*sin(theta);
T(3,3) = cos(alpha)*cos(beta) - sin(alpha)*sin(beta)*sin(theta);
T(3,4) = d*cos(beta) - a*sin(beta)*cos(theta);
...
%--------------------------------------
% dd,dtheta,da,dalpha,beta
dP(1,:) =  [sin(beta), -a*cos(beta)*sin(theta), cos(beta)*cos(theta), 0, d*cos(beta)-a*sin(a)*cos(theta)];
dP(2,:) =  [0, a*cos(theta), sin(theta), 0, 0];
dP(3,:) =  [cos(beta), a*sin(beta)*sin(theta), -sin(beta)*cos(theta), 0, -d*sin(beta)-a*cos(beta)*cos(theta)];
%--------------------------------------
...
dR(:,:,1) = ...
    [0  -cos(beta)*sin(theta)               0    0                                                       -sin(beta)*cos(theta);
     0  -cos(theta)*cos(alpha)*cos(beta)    0    cos(alpha)*sin(beta)+cos(beta)*sin(theta)*sin(alpha)    cos(beta)*sin(alpha)+cos(alpha)*sin(beta)*sin(theta);
     0  cos(theta)*sin(alpha)*cos(beta)     0    sin(theta)*cos(alpha)*cos(beta)- sin(alpha)*sin(beta)   cos(alpha)*cos(beta)-sin(alpha)*sin(beta)*sin(theta)];
...
dR(:,:,2) = ...
    [0  cos(theta)                0    0                         0;
     0  -cos(alpha)*sin(theta)    0    -cos(theta)*sin(alpha)    0;
     0  sin(theta)*sin(alpha)     0    -cos(alpha)*cos(theta)    0];
...
dR(:,:,3) = ...
    [0  sin(beta)*sin(theta)               0    0                                                       -cos(beta)*cos(theta);
     0  cos(theta)*cos(alpha)*sin(beta)    0    cos(alpha)*cos(beta)-sin(beta)*sin(theta)*sin(alpha)    cos(beta)*cos(alpha)*sin(theta)-sin(alpha)*sin(beta);
     0  -cos(theta)*sin(alpha)*sin(beta)   0    -sin(alpha)*cos(beta)-cos(alpha)*sin(theta)*sin(beta)   -cos(alpha)*sin(beta)-sin(alpha)*cos(beta)*sin(theta)];
